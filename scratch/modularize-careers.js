...
const html = fs.readFileSync('public/cloned-pages/careers-body.html', 'utf8');
const dom = new JSDOM(html);
const document = dom.window.document;

const componentsDir = 'components/careers';
...
function toJSX(node) {
    ...
    // Handle custom elements that might break TS
    if (tag === 'hellohire-jobs') {
        const attrs = Array.from(node.attributes).map(a => `${a.name}="${a.value}"`).join(' ');
        return `<hellohire-jobs ${attrs}></hellohire-jobs>`;
    }
...
const ${name}: React.FC = () => {
    return (
        ${jsx.replace('<script src="https://static.hellohireapp.com/embed/hellohire-jobs.js"></script>', '<Script src="https://static.hellohireapp.com/embed/hellohire-jobs.js" />')}
    );
};
...
if (mainContent) {
    ...
    const names = ['CareersTopLogo', 'CareersHeroContent', 'CareersBottomCta'];
    
    sections.forEach((section, index) => {
        const name = names[index] || `CareersSection\${index}`;
        createComponent(name, section);
    });
}

console.log('Careers components created.');
