...
const html = fs.readFileSync('public/cloned-pages/privacy-body.html', 'utf8');
const dom = new JSDOM(html);
const document = dom.window.document;

const componentsDir = 'components/privacy';
...
if (mainContent) {
    const hero = mainContent.querySelector('section.entry-hero');
    if (hero) createComponent('PrivacyHero', hero);
    
    const article = mainContent.querySelector('article');
    if (article) createComponent('PrivacyContent', article);
}

console.log('Privacy components created.');
