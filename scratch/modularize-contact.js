...
const html = fs.readFileSync('public/cloned-pages/contact-body.html', 'utf8');
const dom = new JSDOM(html);
const document = dom.window.document;

const componentsDir = 'components/contact';
...
if (mainContent) {
    ...
    const names = ['ContactHero', 'ContactFormSection', 'ContactInfoSection'];
    
    sections.forEach((section, index) => {
        const name = names[index] || `ContactSection\${index}`;
        createComponent(name, section);
    });
}

console.log('Contact components created.');
