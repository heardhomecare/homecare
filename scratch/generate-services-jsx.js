...
const html = fs.readFileSync('public/cloned-pages/services-body.html', 'utf8');
...
function processNode(node) {
    ...
    for (let i = 0; i < node.attributes.length; i++) {
        ...
        if (name === 'href' || name === 'src' || name === 'srcset') {
            value = value.replace(/https:\/\/uplift-homecare\.com\//g, '/');
            value = value.replace(/index\.html/g, '');
        }
...
const ClonedServicesPage: React.FC = () => {
    return (
...
export default ClonedServicesPage;
`;

fs.writeFileSync('components/raw/ClonedServicesPage.tsx', componentTemplate);
console.log('ClonedServicesPage.tsx regenerated with fixed attributes.');
